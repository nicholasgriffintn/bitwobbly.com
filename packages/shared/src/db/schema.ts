...
export const monitorState = sqliteTable('monitor_state', {
  monitorId: text('monitor_id').primaryKey().references(() => monitors.id),
...
export const statusPageComponents = sqliteTable('status_page_components', {
  statusPageId: text('status_page_id').notNull().references(() => statusPages.id),
...
export const componentMonitors = sqliteTable('component_monitors', {
  componentId: text('component_id').notNull().references(() => components.id),
  monitorId: text('monitor_id').notNull().references(() => monitors.id),
}, (table) => ({
  pk: { primary: true, columns: [table.componentId, table.monitorId] },
}));
