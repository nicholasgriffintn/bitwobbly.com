...
- **Analytics Engine / Iceberg integration** -- The Checker Worker writes check results (latency, status) to the Analytics Engine dataset. The App Worker queries it for monitor metrics and uptime charts. This is not fully implemented / relised yet to produce graphs that are useful to users, it's just a base level at the moment.
- **Issues analytics errors** -- "R2 SQL query failed: {"result":null,"success":false,"errors":[{"code":40003,"message":"invalid SQL: failed to compile SQL query: Schema error: No field named \"NaN\". Valid fields are __ingest_ts, manifest_id, sentry_project_id, project_id, received_at, envelope_id, item_type, event_id, r2_raw_key, item_index, item_length_bytes, sdk_name, sdk_version, sent_at, sent_at_drift_ms, trace_id, trace_public_key, trace_release, trace_environment, trace_user_segment, item_content_type, item_filename, item_attachment_type, item_count, item_platform, event_platform, event_level, event_release, event_environment, event_user_id, event_transaction, event_message."}],"messages":["[40003] invalid SQL: failed to compile SQL query: Schema error: No field named \"NaN\". Valid fields are __ingest_ts, manifest_id, sentry_project_id, project_id, received_at, envelope_id, item_type, event_id, r2_raw_key, item_index, item_length_bytes, sdk_name, sdk_version, sent_at, sent_at_drift_ms, trace_id, trace_public_key, trace_release, trace_environment, trace_user_segment, item_content_type, item_filename, item_attachment_type, item_count, item_platform, event_platform, event_level, event_release, event_environment, event_user_id, event_transaction, event_message."]}"
